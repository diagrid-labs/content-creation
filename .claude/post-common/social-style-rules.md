...
| LinkedIn | 1000 | 3-5, mixed-case, end of post | None | Light (1-2 max, not in first line) | Problem-solution opener, short paragraphs | At the end of the post | `linkedin` |
...
| Dev.to (title 120 / body 2500) | 120 / 2500 | None inline (Dev.to uses front-matter tags, out of scope here) | `@diagrid` (only when natural) | Minimal (0-1) | Blog-style post, technical, neutral | At the end of the post | `dev-to` |
| Medium (title 120 / body 2500) | 120 / 2500 | None inline (Medium uses its own tags, out of scope here) | None | Minimal (0-1) | Blog-style post, technical, neutral (same content as Dev.to) | At the end of the post | `medium` |
...
- Reddit, Dapr Discord, Dev.to, Medium: no inline hashtags
...
### Dev.to
- Blog-style: a title (max 120 chars) and a body (max 2500 chars) structured like a short blog post, not a share blurb.
- Title states the technical detail, not a marketing benefit.
- Structure the body with a short intro that leads with the technical takeaway, then two to four sections with subheadings (`###` or `####`, never `##`, which is reserved for platform sections in the output file), and a closing CTA.
- The post should read as self-contained while still pointing readers to the canonical article at the end.
- Favor a list (bullets or numbered) inside a section when the topic has 3 or more discrete points.
...
### Medium
- Uses the same blog-style content as Dev.to: reuse the Dev.to title and body verbatim. The only difference is the final link, which carries `utm_source=medium`.
- Title (max 120 chars) states the technical detail, not a marketing benefit.
- Body (max 2500 chars) is a short blog post: intro that leads with the technical takeaway, two to four sections with subheadings (`###` or `####`, never `##`), and a closing CTA.
- Link at the end of the body.
- No inline hashtags and no handle mention.
